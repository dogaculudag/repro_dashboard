...
import {
  ...
  returnPreReproToQueue as returnPreReproToQueueService,
...
/**
 * Claim Pre-Repro (Devral): delegates to preRepro.service
 */
export async function claimPreRepro(fileId: string, userId: string) {
  return claimPreReproFile({ fileId, userId });
}

/**
 * Handoff Pre-Repro (Devret): delegates to preRepro.service
 */
export async function completePreRepro(fileId: string, userId: string) {
  return completePreReproFile({ fileId, userId });
}

/**
 * Return Pre-Repro file to queue (Geri Kuyruğa At): delegates to preRepro.service
 */
export async function returnPreReproToQueue(fileId: string, userId: string) {
  return returnPreReproToQueueService({ fileId, userId });
}
